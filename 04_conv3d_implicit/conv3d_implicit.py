"""
Conv3d using implicit im2col — the im2col address computation is fused into
the Triton matmul kernel so no intermediate matrix is ever materialized.

Instead of:
  1. im2col: build a (K, N_positions) matrix in GPU memory
  2. matmul: weight @ im2col_matrix

We do:
  1. A single kernel that loads weight tiles normally (A-side) and computes
     input addresses on-the-fly (B-side) inside the K-loop.

This saves GPU memory (no intermediate matrix) and reduces memory bandwidth.
